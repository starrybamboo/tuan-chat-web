...
- `infra/`
  - 偏底层的“基础设施”：本地 SQLite、WebSocket、文档缓存、BGM、音频缓存等。
- `input/`
  - 输入框、命令面板、工具栏、富文本工具等。
- `message/`
  - 消息渲染相关组件（气泡、预览、附件等）。
...
3) 初始化历史消息（`useChatHistory`，本地 SQLite + Server 补齐）。
...
- 本地 SQLite：`infra/localDb/useChatHistory.ts` 负责消息缓存。
