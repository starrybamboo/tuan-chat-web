import type { DatabaseBlockModel, ListType, ParagraphType } from "@blocksuite/affine/model";
    ...
    const addDatabase = (title: string, group = true) => {
      ...
      paragraphTypes.forEach((type) => {
        const id = store.addBlock(
          "affine:paragraph",
          { type, text: new Text(`Paragraph type ${type}`) },
          databaseId,
        );
        if (richTextId) {
      ...
      listTypes.forEach((type) => {
        const id = store.addBlock(
          "affine:list",
          { type, text: new Text(`List type ${type}`) },
          databaseId,
        );
        if (richTextId) {
          datasource.cellValueChange(
