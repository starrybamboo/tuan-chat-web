...
export async function createTestApp(doc: Store, collection: Workspace) {
  ...
  const defineGlobalGetterOnce = (key: string, getter: () => unknown) => {
  ...
  defineGlobalGetterOnce("host", () => document.querySelector("editor-host"));
  defineGlobalGetterOnce("std", () => document.querySelector("editor-host")?.std);
