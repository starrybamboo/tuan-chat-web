...
import { useGetRoomModuleRoleQuery, useGetRoomRoleQuery } from "../../../../api/hooks/chatQueryHooks";
...
export function AddRoleWindow({
  ...
  const moduleRolesQuery = useGetRoomModuleRoleQuery(roomId ?? -1);
  ...
  const availableRoles = useMemo(() => {
    const roles = addModuleRole ? moduleRoles : userRoles;
    return roles.filter(role => !roomRoles.some(r => r.roleId === role.roleId));
  }, [addModuleRole, moduleRoles, roomRoles, userRoles]);
