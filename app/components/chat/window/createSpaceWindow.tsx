...
export default function CreateSpaceWindow({ onCancel, onSuccess }: CreateSpaceWindowProps) {
  ...
  const [isCreateFlowSubmitting, setIsCreateFlowSubmitting] = useState(false);
  ...
  async function createSpace() {
    ...
    setIsCreateFlowSubmitting(true);
    try {
      ...
      if (typeof newSpaceId === "number" && Number.isFinite(newSpaceId) && newSpaceId > 0) {
        ...
        if (initialImportMessages.length > 0) {
          try {
            const roomsResult = await fetchUserRoomsWithCache(queryClient, newSpaceId);
            const rooms = roomsResult?.data?.rooms ?? [];
            const mainRoom = rooms.find(room => room.roomType === 2)
              ?? rooms.find(room => room.name === "主聊天房")
              ?? rooms[0];
            const mainRoomId = mainRoom?.roomId;
            if (typeof mainRoomId !== "number" || mainRoomId <= 0) {
    ...
    <>
      ...
      ">
        ...
        ">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-1 text-xs text-base-content/55">
                  {initialImportMessages.length > 0
                    ? `已准备 ${initialImportMessages.length} 条，创建后自动导入主聊天房。`
                    : "可先粘贴聊天记录，创建空间后写入主聊天房。"}
                </p>
              </div>
          ...
          <SettingsSection
            ...
            isExpanded={isTrpgSettingsExpanded}
          ...
          <SettingsSection
            ...
            isExpanded={isWebgalSettingsExpanded}
            onToggle={() => setIsWebgalSettingsExpanded(prev => !prev)}
            accessory={(
      ...
      >
        <ImportChatMessagesWindow
          ...
          onClose={() => setIsImportDialogOpen(false)}
          submitLabel={(count, importing) => importing ? "正在保存..." : `保存导入内容 (${count} 条)`}
          successMessage="已准备导入内容"
        />
      </ToastWindow>
    </>
  );
