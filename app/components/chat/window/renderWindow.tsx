        ...
        <>
          ...
          <div className="form-control space-y-2 mb-4">
            <label className="label p-0">
              <span className="label-text text-base-content font-medium">TTS 引擎</span>
            </label>
            <div className="flex gap-2">
              <button
                type="button"
                className={`btn btn-sm flex-1 ${
                  renderProps.ttsEngine === "index"
                    ? "btn-primary"
                    : "btn-outline"
                }`}
                onClick={() => updateRenderProps((draft) => {
                  draft.ttsEngine = "index";
                })}
              >
                IndexTTS2
              </button>
              <button
                type="button"
                className={`btn btn-sm flex-1 ${
                  renderProps.ttsEngine === "gpt-sovits"
                    ? "btn-primary"
                    : "btn-outline"
                }`}
                onClick={() => updateRenderProps((draft) => {
                  draft.ttsEngine = "gpt-sovits";
                })}
              >
                GPT-SoVITS
              </button>
          ...
          {/* GPT-SoVITS 配置 */}
          {renderProps.ttsEngine === "gpt-sovits" && (
            <div className="form-control space-y-3 mb-4 p-3 rounded-lg border border-base-300 bg-base-200">
              <div className="text-sm font-medium text-base-content">GPT-SoVITS 配置</div>
              ...
              <div>
                ...
                <input
                  ...
                  value={renderProps.gptSovitsConfig?.apiUrl || ""}
                  onChange={e => updateRenderProps((draft) => {
                    if (!draft.gptSovitsConfig) {
                      draft.gptSovitsConfig = {
                        apiUrl: "",
                        refAudioPath: "",
                        promptText: "",
                        promptLang: "all_zh",
                    ...
                    draft.gptSovitsConfig.apiUrl = e.target.value;
                  })}
              ...
              <div>
                <label className="label p-0 pb-1">
                  <span className="label-text text-xs">参考音频路径 (服务器端)</span>
                </label>
                <input
                  type="text"
                  title="GPT-SoVITS 参考音频服务器路径"
                  className="input input-sm input-bordered w-full"
                  value={renderProps.gptSovitsConfig?.refAudioPath || ""}
                  onChange={e => updateRenderProps((draft) => {
                    if (!draft.gptSovitsConfig) {
                      draft.gptSovitsConfig = {
                    ...
                    draft.gptSovitsConfig.refAudioPath = e.target.value;
                  })}
              ...
              <div>
                <label className="label p-0 pb-1">
                  <span className="label-text text-xs">提示文本</span>
                </label>
                <input
                  type="text"
                  title="GPT-SoVITS 提示文本"
                  className="input input-sm input-bordered w-full"
                  value={renderProps.gptSovitsConfig?.promptText || ""}
                  onChange={e => updateRenderProps((draft) => {
                    if (!draft.gptSovitsConfig) {
                      draft.gptSovitsConfig = {
                        apiUrl: "http://127.0.0.1:9880",
                    ...
                    draft.gptSovitsConfig.promptText = e.target.value;
                  })}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="label p-0 pb-1">
                    <span className="label-text text-xs">提示文本语言</span>
                  </label>
                  <select
                    title="GPT-SoVITS 提示文本语言"
                    className="select select-sm select-bordered w-full"
                    value={renderProps.gptSovitsConfig?.promptLang || "all_zh"}
                    onChange={e => updateRenderProps((draft) => {
                      if (!draft.gptSovitsConfig) {
                        draft.gptSovitsConfig = {
                          apiUrl: "http://127.0.0.1:9880",
                      ...
                      draft.gptSovitsConfig.promptLang = e.target.value;
                    })}
                  >
                    <option value="all_zh">中文</option>
                    <option value="en">英文</option>
                    <option value="all_ja">日文</option>
                ...
                <div>
                  <label className="label p-0 pb-1">
                    <span className="label-text text-xs">目标文本语言</span>
                  </label>
                  <select
                    title="GPT-SoVITS 目标文本语言"
                    className="select select-sm select-bordered w-full"
                    value={renderProps.gptSovitsConfig?.textLang || "all_zh"}
                    onChange={e => updateRenderProps((draft) => {
                      if (!draft.gptSovitsConfig) {
                        draft.gptSovitsConfig = {
                          apiUrl: "http://127.0.0.1:9880",
                      ...
                      draft.gptSovitsConfig.textLang = e.target.value;
                    })}
                  >
                    <option value="all_zh">中文</option>
                    <option value="en">英文</option>
              ...
              <div className="text-xs text-warning">
                ⚠️ 使用 GPT-SoVITS 时,参考音频路径需要是服务器端可访问的路径
              </div>
            </div>
          )}
