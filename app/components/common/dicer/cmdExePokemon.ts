import type { PokemonSkill } from "@/components/common/dicer/pokemonSkills.d";

// 宝可梦战斗指令文件（cmdExePokemon.ts）
import { CommandExecutor, RuleNameSpace } from "@/components/common/dicer/cmd";
import POKEMON_SKILLS from "@/components/common/dicer/pokemonSkills.json";
...
const cmdRi = new CommandExecutor(
  ...
  async (args: string[], mentioned: UserRole[], cpi: CPI, prop: ExecutorProp): Promise<boolean> => {
    ...
    if (!targetRole || !targetRole.roleId) {
      cpi.sendMsg(prop, "错误：未找到目标角色，请@指定角色或确保当前用户已创建角色");
      return false;
    }
    ...
    if (!curAbility?.ability && !curAbility?.basic && !curAbility?.skill) {
      cpi.sendMsg(prop, `错误：${targetRole.roleName}的角色属性未设置`);
      return false;
    }
    ...
    if (!speedValueStr || Number.isNaN(Number(speedValueStr))) {
      cpi.sendMsg(prop, `错误：${targetRole.roleName}未设置“速度”属性`);
      return false;
    }
...
const cmdattack = new CommandExecutor(
  ...
  async (args: string[], mentioned: UserRole[], cpi: CPI, prop: ExecutorProp): Promise<boolean> => {
    ...
    if (!skillName || !targetRole) {
      cpi.sendMsg(prop, "格式错误！正确格式：.battle [技能名] @目标宝可梦");
      return false;
    }
    ...
    if (!skill) {
      cpi.sendMsg(prop, `未找到技能“${skillName}”`);
      return false;
    }

    // 执行战斗逻辑（见下方步骤）
    await executeBattle(attackerRole, targetRole, skill, cpi, prop);
    return true;
  },
);
...
async function executeBattle(
  ...
  //   const hitRate = skill.accuracy;
  //   const hitRoll = Math.floor(Math.random() * 100) + 1; // 1-100随机数
  //   if (hitRoll > hitRate) {
  //     cpi.sendMsg(prop, `${attacker.roleName}使用的${skill.name}没有命中！`);
  //     return;
  //   }
  // }
  ...
  // 3. 伤害计算（变化技能无伤害）
  if (skill.category === "status") {
    cpi.sendMsg(prop, `${attacker.roleName}使用了${skill.name}！${skill.effect}`);
    return;
  }
  ...
  // cpi.sendMsg(prop, `${attacker.roleName}使用了${skill.type}！`);
  // cpi.sendMsg(prop, `攻击属性1：${attackerType1}，攻击属性2：${attackerType2}\n`);
  // cpi.sendMsg(prop, `防守属性1：${defenderType1}，防守属性2：${defenderType2}\n`);
  ...
  // 3.4 额外倍率（如随机浮动10%-15%，或技能特效加成）
  // const randomBonus = 1 + (Math.floor(Math.random() * 5) + 10) / 100; // 1.1-1.15随机
  ...
  cpi.sendMsg(prop, `攻击方roleid：${attacker.roleId},防守方roleid：${defender.roleId}`);
  // cpi.sendMsg(prop,`"原始HP：" ${defenderHp}, "计算后HP：" ${newHp}, "最终存储的HP："${UNTIL.getRoleAbilityValue(defenderAbility, "hp")}`);

  // 5. 输出战斗结果
  cpi.sendMsg(prop, `${attacker.roleName}对${defender.roleName}使用了${skill.name}！\n`
  + `伤害计算：\n`
