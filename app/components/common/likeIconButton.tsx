...
import { useGetLikeCountQuery, useIsLikedQuery, useLikeMutation, useUnlikeMutation } from "../../../api/hooks/likeQueryHooks";
...
export default function LikeIconButton({ targetInfo, className, icon, direction = "column" }: LikeIconButtonProps) {
  ...
  const likeCount = likeCountQuery.data?.data ?? 0;
  ...
  // 计算显示的点赞数量
  const displayLikeCount = React.useMemo(() => {
    let count = likeCount;
    if (userId != null) {
      count = likeCount;
    }
  ...
  return (
    ...
    >
      ...
      <span className="text-xs mt-1">
        {likeCountQuery.isLoading ? "..." : displayLikeCount}
      </span>
    </button>
  );
}
