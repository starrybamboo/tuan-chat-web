...
import { getEntityListByType } from "../repositoryUtils";
...
export default function Roles({ repositoryId }: { repositoryId: number }) {
  const { data: repositoryInfo } = useRepositoryInfoQuery(repositoryId);

  // 获取所有角色，name为唯一标识符
  const roleList = getEntityListByType(repositoryInfo, "role");
