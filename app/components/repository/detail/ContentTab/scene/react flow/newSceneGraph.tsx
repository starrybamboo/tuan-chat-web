...
import { getEntityListByType, mapEntitiesByVersionId } from "../../../../detail/repositoryUtils";
...
export default function NewSceneGraph(props: NewSceneGraphProps) {
  ...
  const { data: repositoryInfo } = useRepositoryInfoQuery(Number(repositoryId!));
  // 根据sceneMap和增强场景数据生成节点和边
  const { initialNodes, initialEdges } = useMemo(() => {
    // 如果还在加载或没有数据，返回空数组
    if (!repositoryInfo || isLoading) {
    ...
    const rawSceneMap = repositoryInfo?.data?.repositoryMap?.sceneMap || {};

    // 2. enhancedScenes: 所有场景类型的实体列表，每个元素包含 id、versionId、name、entityInfo 等完整字段
    //    作用: 提供场景的详细数据（名称、描述、包含的角色/物品/地点等），用于填充节点展示内容
    const enhancedScenes = getEntityListByType(repositoryInfo, "scene");

    // 3. sceneMapByVersionId: 将场景实体按 versionId 建立索引，方便后续通过 versionId 快速查找对应实体
    //    数据结构: Record<string, RepositoryEntityResponse>，例如 { "101": {id:1, versionId:101, name:"开场",...}, ... }
    ...
    const nodes: Node[] = sceneIdList.map((sceneVersionId, index) => {
      ...
      return {
        ...
        data: {
          repositoryInfo: repositoryInfo.data?.responses,
          label: sceneName,
          idx: index,
  ...
  }, [repositoryInfo, isLoading, isMobile]);
