...
export class PremiereExporter {
  ...
  public async processMessages(
    ...
    for (const item of messages) {
      ...
      // Pre-calculate Background URL to determine if this is a "pure background switch" message
      ...
      // Note: We haven't run TTS yet but if content is empty, TTS won't run.
      // Condition: Has BG URL AND (No content OR Content is just the URL we extracted)
      const isContentEmpty = !msg.content || (bgUrl && msg.content === bgUrl) || msg.content.trim() === '';
      const isResultPureBackground = bgUrl && isContentEmpty;

      if (isResultPureBackground) {
          // Handle Pure Background Switch: Update active clip, do NOT advance time
          if (bgUrl) { // Redundant check for TS
             const fileId = this.addResource(bgUrl, "image", `bg_${msg.messageId}`);
             
             // Close previous BG clip
             if (activeBgClip && activeBgClip.fileId !== fileId) {
                 activeBgClip.end = currentTime; // Ends NOW
                 this.tracks.background.push(activeBgClip);
                 activeBgClip = null;
             }

             // Start new BG clip if not already active
             if (!activeBgClip) {
                 let bgScale = 100;
                 try {
                 ...
                 activeBgClip = {
                     ...
                     start: currentTime, // Starts NOW
                     end: currentTime, // Will be extended by next message
                     in: 0, out: 0,
                     fileId, type: "video",
                     position: POS.BG, 
                     scale: bgScale
      ...
      const contentDuration = audioDuration ?? this.estimateDuration(msg.content);
      const bufferDuration = vocalFileId ? 0.5 : 0;
      const startTime = currentTime;

      // Update currentTime using the rounded frame logic to prevent drift
      // Ideally, the next clip should start exactly where the current one ends in FRAMES.
      // Premiere XML is frame-based. If we accumulate float seconds, Math.round might jump differently than cumulative addition.
      const currentStartFrame = Math.round(startTime * this.fps);
      const durationFrames = Math.round((contentDuration + bufferDuration) * this.fps);
      const nextStartFrame = currentStartFrame + durationFrames;
      
      // Re-adjust displayEndTime to match this frame alignment exactly
      const alignedDisplayEndTime = nextStartFrame / this.fps; 
      
      // Force currentTime to align with the frame grid for the next iteration
      currentTime = alignedDisplayEndTime;

      if (vocalFileId) {
          this.tracks.voice.push({
              id: this.getNextClipId(),
              name: `Voice ${msg.messageId}`,
              // Voice clip ends when audio content ends (no silence stretch)
              start: startTime, 
              end: startTime + contentDuration, 
              in: 0, 
              out: contentDuration,
      ...
      // Dialogs MUST align with the Visuals Duration
      if (msg.content) {
        this.dialogs.push({
          id: msg.messageId,
          startTime,
          endTime: alignedDisplayEndTime, // Sync with visual end time
          text: msg.content
        });
      }

      // 1. Background (Part 2: If message has content AND background change)
      
      if (!isResultPureBackground) {
           // Reuse the bgUrl if calculated above
           if (!bgUrl) {
               // Re-run checks if not already set (e.g. maybe content wasn't empty but still has BG logic inside?)
           }
          ...
          if (bgUrl) {
             ...
             if (!activeBgClip) {
                 ...
                 activeBgClip = {
                     ...
                     name: `BG ${msg.messageId}`,
                     start: startTime, 
                     end: alignedDisplayEndTime, 
                     in: 0, 
                     out: contentDuration + bufferDuration,
                     fileId, type: "video",
                     position: POS.BG, 
                     scale: bgScale
                 };
             } else {
                 activeBgClip.end = alignedDisplayEndTime;
             }
      ...
      if (msg.roleId && fetchRole) {
          try {
             ...
             // Check if NPC
             if (roleInfo && typeof (roleInfo as any).type === 'number') {
                 // 0: Role, 1: Dice, 2: NPC.
                 if ((roleInfo as any).type === 2) isNpc = true;
             }
      ...
      if (avatarUrl) {
          ...
          // NPC always 150% (unless correction modification applies on top)
          let baseScale = 100;
          if (isNpc) {
              baseScale = 150;
          } else {
              baseScale = isDefaultAvatar ? 100 : 150;
          }
          ...
          if (!currentClip || currentClip.fileId !== fileId) {
              ...
              activeFigureClips[trackKey]!.end = alignedDisplayEndTime;
          }
      } else {
    ...
    // Close open clips
    if (activeBgClip) this.tracks.background.push(activeBgClip);
    for (const key of (Object.keys(activeFigureClips) as Array<keyof typeof activeFigureClips>)) {
        if (activeFigureClips[key]) this.tracks.figures[key].push(activeFigureClips[key]!);
    }
  }
  ...
  public generateXML(): string {
    const renderTrack = (clips: TrackClip[]) => {
        if (clips.length === 0) return "<track><enabled>TRUE</enabled><locked>FALSE</locked></track>";
...
<sequence id="sequence-1">
  ...
  public generateSRT(): string {
    const formatTime = (seconds: number) => {
      // Use standard SMTP TE timecode calculation logic to match Premiere's interpretation
      // However, SRT uses milliseconds. 
      // We align to the exact frame start time.
      const frameIndex = Math.round(seconds * this.fps);
      
      // Calculate strict time from frame index
      const totalSeconds = frameIndex / this.fps;
      
      const date = new Date(0);
      date.setMilliseconds(Math.round(totalSeconds * 1000));
    ...
    return this.dialogs.map((d, index) => {
      // Ensure we use the exact same rounding logic as the XML generator
      // The clip start in XML is Math.round(start * fps)
      // The timeline runs at 'fps'. 
      // So the subtitle must start EXACTLY at (frameStart / fps).
