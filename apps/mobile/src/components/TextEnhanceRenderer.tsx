...
function cssToTextStyle(css: Record<string, string>): TextStyle {
  ...
  if (css.fontWeight)
    style.fontWeight = css.fontWeight as TextStyle["fontWeight"];
  if (css.fontStyle)
    style.fontStyle = css.fontStyle as TextStyle["fontStyle"];
  if (css.textDecorationLine)
    style.textDecorationLine = css.textDecorationLine as TextStyle["textDecorationLine"];
  ...
  if (css.fontSize) {
    const parsed = Number.parseFloat(css.fontSize);
  ...
  if (css.letterSpacing) {
    const parsed = Number.parseFloat(css.letterSpacing);
    if (!Number.isNaN(parsed))
      style.letterSpacing = parsed;
  }
  if (css.opacity) {
    const parsed = Number.parseFloat(css.opacity);
    if (!Number.isNaN(parsed))
      style.opacity = parsed;
  }
  if (css.lineHeight) {
    const parsed = Number.parseFloat(css.lineHeight);
    if (!Number.isNaN(parsed)) {
