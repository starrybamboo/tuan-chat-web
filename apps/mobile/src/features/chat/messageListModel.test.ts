...
describe("messageListModel", () => {
  ...
  it("保留普通可见消息", () => {
    ...
    expect(isVisibleRoomMessage(makeMessage({ messageType: MESSAGE_TYPE.STATE_EVENT, status: 0 }))).toBe(true);
  });

  it("从列表数据源中过滤已删除和 EFFECT 消息", () => {
  ...
  it("构建回复查找表时排除已删除和 EFFECT 消息", () => {
    ...
    const map = buildVisibleMessageMap([item(visible), item(deleted), item(effect)]);
