...
describe("mobile media-url", () => {
  ...
  it("音频和视频请求任意高档位都会回退到 low", () => {
    expect(mediaFileUrl(12, "audio", "high")).toBe("https://tuan.chat/media/v1/files/012/12/audio/low.webm");
    expect(mediaFileUrl(12, "audio", "original")).toBe("https://tuan.chat/media/v1/files/012/12/audio/low.webm");
  ...
  it("头像缩略图继续使用 low 档位", () => {
    ...
    expect(avatarThumbUrl(null)).toBe("");
  });
});
