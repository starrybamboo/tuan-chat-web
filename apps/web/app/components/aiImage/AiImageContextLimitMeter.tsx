...
type AiImageContextLimitMeterProps = {
  localUsed: number;
  totalUsed: number;
  remaining: number;
  overflow: number;
  ...
  return (
    <div className={className}>
      <div className="relative">
        <div
          className="
            peer relative h-1 w-full overflow-hidden rounded-md
            bg-base-content/12
          "
          role="img"
        ...
        >
          {totalPercent > 0
            ? (
                <div
                  className="
                    absolute inset-y-0 left-0 rounded-md bg-base-content/18
                  "
            ...
            ? (
                <div
                  className={`
                    ...
                    ${overflow > 0 ? `bg-warning` : `bg-base-content/80`}
                  `}
        ...
        {(footerLabel || footerHint)
          ? (
              <div className="
                mt-[13px] flex items-center gap-1 text-xs leading-none
                  ...
                  ? (
                      ...
                      >
                        <span className="
                          ...
                          border border-base-content/16 text-[10px] font-medium
        ...
        {footerHint && footerTooltipState.visible
          ? (
              <div
                className="
                  pointer-events-none fixed z-30 flex min-h-[80px] w-[300px]
                  items-center rounded-xl border border-base-300 bg-base-100
                  px-3 py-2 text-xs leading-5 text-base-content/72 shadow-xl
                "
                style={{
          ...
          <div className="mt-3 space-y-2">
            {rows.map(row => (
              <div key={row.label} className="
                flex items-center justify-between gap-4 text-[11px] leading-none
