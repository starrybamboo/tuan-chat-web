...
7) `app/components/chat/infra/localDb/useChatHistory.ts`
   - 历史消息缓存与同步逻辑。
...
- `infra/`
  - 偏底层的“基础设施”：sqlite-wasm OPFS 本地消息缓存、WebSocket、文档缓存、BGM、音频缓存等。
- `input/`
  - 输入框、命令面板、工具栏、富文本工具等。
- `message/`
  - 消息渲染相关组件（气泡、预览、附件等）。
...
3) 初始化历史消息（`useChatHistory`，本地消息缓存 + Server 补齐）。
...
- 本地消息缓存：`infra/localDb/useChatHistory.ts` 负责 sqlite-wasm OPFS 消息缓存。
...
- 本地快速展示
- 服务器补齐
- sqlite-wasm OPFS 持久化；不可用时禁用本地缓存并提示

### 4.3 实时消息同步（WebSocket）

```
WebSocket → roomMessageEvents
         → useChatHistory
         → chatHistory.addOrUpdateMessages
         → ChatFrameList 渲染
```

`useChatHistory` 会监听房间消息事件，检测消息缺口（syncId 断层）并补拉历史；`useChatFrameMessages` 只负责按通用可见性规则过滤消息。
...
- 与服务器补齐
- 页面可见性变化时刷新
- sqlite-wasm OPFS 不可用时派发本地缓存不可用提示
