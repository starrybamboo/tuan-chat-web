...
import { detectIncomingRoomMessageGapStart } from "../infra/localDb/useChatHistory";
...
describe("detectIncomingRoomMessageGapStart", () => {
  it("本地 latestSyncId 已覆盖删除等隐藏消息时不补拉 history", () => {
    expect(detectIncomingRoomMessageGapStart({
      currentMessages: [message(1, 100)],
      incomingMessages: [message(2, 102)],
  ...
  it("收到真正跳号的新消息时返回缺失区间", () => {
    expect(detectIncomingRoomMessageGapStart({
      currentMessages: [message(1, 100)],
      incomingMessages: [message(2, 103)],
  ...
  it("收到已知 messageId 的更新时不按较大 syncId 误补洞", () => {
    expect(detectIncomingRoomMessageGapStart({
      currentMessages: [message(1, 100)],
      incomingMessages: [message(1, 103)],
      latestHistorySyncId: 100,
    })).toBeNull();
  });
});
