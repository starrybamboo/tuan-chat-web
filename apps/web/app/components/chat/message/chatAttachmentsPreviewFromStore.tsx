      ...
      <div className="flex flex-row items-center gap-x-3 overflow-x-auto">
        <AnimatePresence mode="popLayout">
          ...
          {emojiUrls.map((url, index) => (
            <motion.div key={url} layout {...attachmentItemMotionProps}>
              <BetterImg
                src={url}
                className="h-12 w-max rounded"
                onClose={() => {
          ...
          {fileAttachments.map((file, index) => {
            ...
            return (
              <motion.div className="relative group shrink-0 max-w-48" key={fileKey} layout {...attachmentItemMotionProps}>
                <div
                  className="
                    h-12 rounded bg-base-200 border border-base-300 px-2 pr-6
                    min-w-28 max-w-48 flex items-center gap-1.5
                  "
                  title={file.name}
                >
                  {isVideo
