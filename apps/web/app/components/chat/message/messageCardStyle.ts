...
export const CHAT_MESSAGE_BUBBLE_BASE_CLASS = "relative max-w-[calc(100vw-5rem)] sm:max-w-md break-words rounded-lg px-3 sm:px-4 py-1.5 sm:py-2 shadow-sm sm:shadow text-base sm:text-sm lg:text-base transition-[background-color,border-color,box-shadow,color] duration-200 motion-reduce:transition-none";
...
const CHAT_MESSAGE_HOVER_TOOLBAR_BASE_CLASS = "absolute top-2 right-2 z-30 flex items-center gap-1 rounded-full border border-base-300/80 bg-base-200/95 px-1.5 py-1 shadow-lg backdrop-blur-sm transition-[opacity,transform,background-color,border-color,box-shadow] duration-150 motion-reduce:transition-none motion-reduce:transform-none";

/**
 * 小屏触摸设备会让 hover 态滞留，工具条只在桌面 hover 时出现。
 */
