...
import WorkflowItemDetail from "./workflowItemDetail";

type WorkflowSceneNodeProps = {
  data: {
    label: string;
    idx: number;
    description?: string;
...
function WorkflowSceneNode({ data, selected }: WorkflowSceneNodeProps) {
  ...
  return (
    <>
      ...
      className="absolute top-full left-1/2 transform -translate-x-1/2 mt-1"        style={{ pointerEvents: "none" }}      >        <span className="text-xs font-semibold text-gray-700 whitespace-nowrap">          {data.label}        </span>      </div> */}
        ...
        >
          <div className={data.isMobile ? "flex h-full w-full flex-col" : "w-[50vw]"}>
            {data.isMobile && (
            ...
            <div className={data.isMobile ? "min-h-0 flex-1 overflow-auto" : ""}>
              { data.children || (
                <WorkflowItemDetail
                  itemName={data.label}
...
export default WorkflowSceneNode;
