...
type MaterialComposerAttachmentsPreviewProps = {
  roomId: number;
}
        ...
        {fileAttachments.map((file, index) => {
          ...
          return (
            <div className="relative max-w-48 flex-shrink-0 group" key={fileKey}>
              <div
                className="
                  flex h-12 min-w-28 max-w-48 items-center gap-1.5 rounded
                  border border-base-300 bg-base-200 px-2 pr-6
                "
                title={file.name}
              >
                {isVideo
              ...
              <div
                className="
                  absolute bottom-0 left-0 right-0 rounded-b bg-black/50 px-1
              ...
              <button
                type="button"
                className="
                  absolute -right-1 -top-1 z-10 rounded-full bg-base-100 shadow
                  transition-colors
