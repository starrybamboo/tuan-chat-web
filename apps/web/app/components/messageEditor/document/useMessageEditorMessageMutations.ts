...
import type { MessageEditorCommandTransaction } from "./messageEditorCommands";
import type {
  MessageEditorHistoryEntry,
  MessageEditorHistoryManager,
} from "./messageEditorHistoryManager";

import { ensureMessageEditorMessages } from "./messageEditorTransforms";
...
type MessageEditorMessageMutationsResult = {
  commitTransaction: <T>(transaction: MessageEditorCommandTransaction<T>) => T;
  getCurrentMessages: () => MessageEditorMessage[];
  messages: MessageEditorMessage[];
  messagesRef: RefObject<MessageEditorMessage[]>;
...
export default function useMessageEditorMessageMutations({
  ...
  const commitTransaction = useCallback(<T,>(transaction: MessageEditorCommandTransaction<T>) => {
