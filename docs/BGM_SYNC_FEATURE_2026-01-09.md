...
- 处理点：群聊消息推送（type=4）入本地缓存前，遍历新增消息：
  ...
  - `SYSTEM + content 包含 [停止BGM]` → `useBgmStore.getState().onBgmStopFromWs(...)`
...
- 协议当前依赖 SYSTEM 文本标记 `[停止BGM]`：如果担心误触发/国际化，建议升级为专用 messageType 或在 extra 中添加结构化字段。
