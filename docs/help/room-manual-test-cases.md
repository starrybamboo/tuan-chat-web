...
- 前置条件：焦点位于消息输入框。
- 输入与操作：按`Tab`。
- 预期输出：不触发 AI 改写，保持浏览器默认的焦点切换行为。
- 截图位：`【截图位-25：输入框 Tab 焦点切换】`

### TC-26 导入文本对话
- 前置条件：准备形如`[角色名]：内容`或`<角色名>：内容`的文本。
...
### TC-30 插入消息模式
- 前置条件：已有历史消息。
- 输入与操作：对某消息点`插入消息`，再发送新消息。
- 预期输出：输入区出现“插入消息中”提示；新消息位置落在目标消息后。
...
- 前置条件：任意可操作消息。
- 输入与操作：右键消息。
- 预期输出：可见`打开副窗口/多选/回复/删除(有权限)/添加标注/插入消息/编辑文本/添加到表情(图片)`等项。
- 截图位：`【截图位-38：消息右键菜单】`
...
5. 打开`房间成员`与`房间角色`抽屉。
6. 切换消息样式（气泡/传统）。
7. 右键一条消息，走`回复`与`插入消息`。
8. 打开副窗口，确认房间内容展示正常。
9. 开启WebGAL联动，发送1条特效或 `.st hp -2` 这类状态消息。
