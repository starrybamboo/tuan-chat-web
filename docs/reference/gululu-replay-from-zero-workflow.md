...
应作为角色头像或立绘的图片必须先做源头分类，不能在导入后混用：

- `character-avatar-bust` / `character-avatar-chat`：头像源图。聊天头像用该图；WebGAL 舞台上的 `spriteFileId` 也用该图本身，但按头像/半身图规则计算 `spriteTransform`。不要自动挂同角色的其他立绘。
- `character-sprite` / `role-sprites/<角色>/`：立绘源图。`avatarFileId` 和 WebGAL 舞台上的 `spriteFileId` 都使用该立绘图本身；区别只在舞台 `spriteTransform` 必须按立绘规则计算。
- `manga-avatar`：漫画头像源图。聊天头像用该漫画头像；WebGAL 舞台可以用该图本身作为头像式 sprite，但按漫画/框图规则保守缩放和定位，不抠图、不透明化、不当作角色立绘。

不作为头像、立绘，也不进入演出消息的图片：
...
- 头像、立绘、`manga-avatar` 要分开处理；分类来自图片审查结果，不靠同角色资源互相替代。
- 同类图片在舞台上的视觉高度应一致。
- 图片应位于对话框之上，不遮挡文本框。
- 不能把漫画大图强行当立绘缩放到舞台中央。
- 头像源图的 WebGAL sprite 就是头像源图本身；不能因为同角色存在立绘，就把头像槽位的 `spriteFileId` 改成那张立绘。
- 立绘源图才走立绘渲染，默认按 cowboy shot 方向处理，让头部到大腿/膝上成为舞台主体。

建议分类：

```text
character-avatar-bust/chat   头像或头像差分；avatarFileId 和 spriteFileId 均指向该头像图，使用头像/半身 transform
character-sprite             立绘；avatarFileId 和 spriteFileId 均指向 role-sprites 透明图，使用 stage-sprite/cowboy shot transform
manga-avatar                 漫画头像；avatarFileId 和 spriteFileId 均指向漫画头像原图，使用 framed-avatar transform
