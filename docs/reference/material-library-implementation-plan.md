...
- 前端：`patchInsertMessages`
- 后端：`ChatController.patchRoomMessages`（`POST /chat/message/patch`）

因此“背景图 + BGM 一键切换”不需要额外发明消息模型，只要把素材条目映射成复合 patch 的 `insert` operations 即可。
