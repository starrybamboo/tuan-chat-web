...
<clear>:bg
<clear>:all
[烈.震惊,丰聪耳神子.闭眼平静]:多人同框对白
[烈(60).震惊]:透明度参数会写入 message.webgal.transform.alpha
```
...
其中 `<bubble>` 目前降级为普通旁白消息，不是舞台浮动气泡对象；`[角色(60).差分]` 的透明度参数会保存在团剧共创通用 transform 字段 `message.webgal.transform.alpha`，实时预览和发布导出都会把它编译进立绘 transform 的 alpha。
...
```json
{
  "package": {
    ...
    "description": "sample-replay 通用素材"
  },
  "media": {
    "backgrounds": {
      "永远亭夜晚": {
...
- 多角色同框对白可写 `[主说话人.差分,陪同角色.差分]:正文`；导入器会先为陪同角色生成空白切换消息并加 `dialog.next`，再生成主说话人的正文消息。省略差分时只允许在同一组内沿用最后一个显式差分，例如 `[烈,丰聪耳神子.默认]`。
- `[角色(60).差分]` 里的透明度会解析为 `0.6` 并写入 `message.webgal.transform.alpha`，实时预览和发布导出都会把它编译进立绘 transform 的 alpha。
...
- 对白或旁白正文末尾的 `{*}` 会作为回声工坊音频占位剥离；`{音效}` / `{音效;*时间}` 会额外生成一条 `sys:se` 素材消息，素材名为花括号内的音效名，时间参数目前只参与解析、不驱动播放偏移。
...
- `<set:BGM>:战斗曲`：兼容回声工坊设置行，等价于 `<sys:bgm>:战斗曲`。
- `<sys:se>:挥刀音效`：从素材包查 SE 素材 `挥刀音效`，生成 `SOUND + sys:se` 消息。
- `<sys:cg>:开场图` / `<image.show>:人物卡展示图`：从素材包查图片素材，生成对应图片展示消息。
- `<background><replace=30>:永远亭夜晚`：等价于背景素材消息，并额外附加背景进入动画与速度 annotation；`replace` 的具体数值会被归一到 `background.speed.fast/normal/slow`。
...
- 元组格式固定为 `(描述,面数,检定值,出目)`。
- 导入后仍然编译成团剧共创 `diceTurn`：骰前 `command` 为描述、空骰式和检定值；骰后 `reply` 为实际出目和成功/失败结算。
- 该形式只用于兼容旧脚本；安科导入仍推荐使用块语法，以便保存选项和多条骰后回复。

## 状态事件

HP 变更使用团剧共创 `STATE_EVENT`，不会伪装成普通旁白：
...
- 第一项是当前房间已有角色名；RGL 消息导入阶段找不到角色会失败，不自动创建角色。
- `-2` / `+6` 编译为 `hp sub/add`；`10` 编译为 `hp set`；`10,20` 或 `hp,10,20` 编译为 `hp set 10` 和 `hpm set 20`。
- 生成消息的 `messageType` 是 `STATE_EVENT`，`extra.stateEvent.source.commandName` 为 `hitpoint`。
...
- `<bubble>` 暂时降级为普通旁白消息，用于保留文本，不表示舞台浮动气泡，也不会自动追加 `dialog.next`。
- `<animation>` 接受一个动画别名和一个可选站位别名，编译成现有 `figure.anim.*` / `figure.pos.*` annotation；它没有角色绑定时只能作为控制消息进入时间线，完整目标角色动画仍推荐写在角色对白上，例如 `[烈.震惊]<shake><left-center>:`。
