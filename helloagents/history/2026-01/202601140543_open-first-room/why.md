...
## 需求背景
当前在空间模式首次进入（URL 缺失 roomId）时，前端会在房间列表尚未加载完成的阶段触发默认选房间逻辑，导致跳转到 `/chat/<spaceId>/null`。该 URL 会阻断后续“缺失 roomId 时自动选择第一个房间”的逻辑，从而无法进入首个房间。
...
- **缓解:** 仅在 `rooms.length>0` 且 `urlRoomId` 缺失/无效时触发，避免空列表与已选房间重复跳转
