...
目录: `helloagents/plan/202601171745_blocksuite-mention-insert-fix/`

---

## 1. 修复插入与关闭弹窗
- [√] 1.1 将成员提及插入改为使用 `inlineEditor`（与上游 linked-doc 插入一致），先 `abort()` 清理 `@query` 再插入 mention，避免弹窗不关闭/残留 query
- [√] 1.2 为 linked-doc popover 的 doc action 与 member mention action 增加短窗口去重（避免同一次确认触发多次 action）
