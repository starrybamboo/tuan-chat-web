...
Ŀ¼: `helloagents/plan/202601252021_doc_message_card/`

---

## 1. 消息协议与渲染
- [√] 1.1 在 `app/types/voiceRenderTypes.ts` 增加 `MESSAGE_TYPE.DOC_CARD`，验证 why.md#需求-发送文档卡片消息-场景-点击发送按钮发送文档卡片
- [√] 1.2 新增文档卡片渲染组件（标题/封面/ժҪ + 点击弹窗只读预览），并在 `app/components/chat/message/chatBubble.tsx` 接入，验证 why.md#需求-发送文档卡片消息-场景-点击发送按钮发送文档卡片
...
## 3. 发送校验与同一 space 限制
- [√] 3.1 发送前校验 docId 类型与当前 space 一致性（必要时降级提示），验证 why.md#需求-ͬһ-space-限制-场景-在非同一-space-场景触发拖拽/发送
