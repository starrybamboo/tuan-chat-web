...
### 需求: KP 可切换全部角色发言
**模块:** chat
KP 在角色选择面板可选择房间内所有角色（含其他玩家角色与 NPC），非 KP 仍仅展示自己角色。
...
- 能力：文档列表（按 tag 分组）、新建文档、设置/清除标签、删除、点击打开 Blocksuite 编辑
  - 文档标题修改：在文档内部通过 `tc_header` 修改，前端监听后同步到后端
- 拖拽复制：将“聊天消息列表的文档卡片消息”拖到“我的文档”抽屉的标签区域，松开后会创建一份 `space_user_doc` 副本并写入标签（未命中标签则归入未分类）
- 数据：对接 `/space/userDoc/*`（文档列表与 tag 分类）
