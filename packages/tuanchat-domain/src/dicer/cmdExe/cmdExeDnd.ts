...
import dndSpellsData from "./dndSpellsData.json";
...
let dndSpellsCache: DndSpell[] | null = null;

async function loadDndSpells(): Promise<DndSpell[]> {
  if (!dndSpellsCache) {
    const data = dndSpellsData as unknown;
    if (!Array.isArray(data)) {
      throw new TypeError("DND spells asset is not an array");
    }
    dndSpellsCache = data as DndSpell[];
  }
  return dndSpellsCache;
}
