import type { CPI, RoleAbility, UserRole } from "../types";
...
export async function executeStShowCommand(
  ...
  cpi.replyMessage(formatRoleAbilityCardReply(curAbility, role.roleName || "当前角色", showProps));
  return true;
}

function formatRoleAbilityCardReply(ability: RoleAbility, roleName: string, requestedKeys: string[]): string {
  ...
  const lines = sections.flatMap(([title, values]) => {
    const rows = Object.entries(values ?? {})
      .filter(([key]) => requested.size === 0 || requested.has(key))
      .map(([key, value]) => `${key}: ${value}`);
    return rows.length > 0 ? [`【${title}】`, ...rows] : [];
  });
  return lines.length > 0 ? [`${roleName}的属性卡`, ...lines].join("\n") : `${roleName}没有可显示的属性。`;
}
