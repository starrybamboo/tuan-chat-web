import type { CPI, UserRole } from "../types";

import { CommandExecutor } from "../cmd";
...
export function parseWwCommandArgs(args: string[], getValue?: (key: string) => number): WwCommandOptions {
  ...
  if (normalized.length > 0) {
    ...
    if (/^\d+$/.test(normalized)) {
      ...
      if (getValue) {
        ...
        // 替换属性名为数值
        // 匹配中文、英文变量名
        const resolvedExpr = normalized.replace(/([\u4E00-\u9FA5a-z_]+)/gi, (match) => {
  ...
  if (sides < 2)
    sides = DEFAULT_SIDES;
  if (explodeAt <= 1)
    explodeAt = MIN_EXPLODE_AT; // 避免死循环
...
export function rollWw(options: WwCommandOptions, rng?: () => number): WwRollResult {
  ...
  while (currentPool > 0 && totalRollsCount < MAX_TOTAL_ROLLS) {
    ...
    for (let i = 0; i < currentPool; i++) {
      if (totalRollsCount >= MAX_TOTAL_ROLLS)
        break;
...
export function formatWwResultMessage(options: WwCommandOptions, result: WwRollResult, operatorName: string = "当前角色"): string {
  // 构建详细指令串
  const paramsStr = `${options.explodeAt !== 10 ? `a${options.explodeAt}` : ""}${options.successAt !== 8 ? `k${options.successAt}` : ""}${options.sides !== 10 ? `m${options.sides}` : ""}`;
