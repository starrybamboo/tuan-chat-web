...
foreach ($variantCachePath in $variantCachePaths) {
    if (Test-Path $variantCachePath) {
        Remove-Item -LiteralPath $variantCachePath -Recurse -Force
    }
}
...
try {
    ...
    # Assemble the requested variant; Release embeds the JS bundle, while Debug uses Metro.
